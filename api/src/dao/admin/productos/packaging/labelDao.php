...
class labelDao
{
    ...
    public function findAllLabel()
    {
        $connection = Connection::getInstance()->getConnection();
        $stmt = $connection->prepare("SELECT * FROM etiqueta");
        $stmt->execute();
        $this->logger->info(__FUNCTION__, array('query' => $stmt->queryString, 'errors' => $stmt->errorInfo()));
        $label = $stmt->fetchAll($connection::FETCH_ASSOC);
        $this->logger->notice("etiquetas Obtenidos", array('etiqueta' => $label));
        return $label;
    }
    ...
    public function updateLabel($dataLabel)
    {
        $connection = Connection::getInstance()->getConnection();
    ...
    {
        $connection = Connection::getInstance()->getConnection();
        $stmt = $connection->prepare("DELETE FROM etiqueta WHERE id = :id");
        $stmt->execute(['id' => $id]);
        $this->logger->info(__FUNCTION__, array('query' => $stmt->queryString, 'errors' => $stmt->errorInfo()));
    }
}