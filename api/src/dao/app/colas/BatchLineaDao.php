...
class BatchLineaDao
{
  ...
  {
    $connection = Connection::getInstance()->getConnection();
    $stmt = $connection->prepare("SELECT DISTINCT batch.id_batch, date_add(batch.fecha_programacion, interval 3 day) AS fecha_programacion, DATE_ADD(batch.fecha_programacion, INTERVAL 13 DAY) AS fecha_envasado, batch.numero_orden, 
