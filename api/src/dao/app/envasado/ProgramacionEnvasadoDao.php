...
class ProgramacionEnvasadoDao
{
    ...
    public function findAverageCapacidadEnvasado()
    {
        ...
        $stmt = $connection->prepare("SELECT * FROM ( SELECT se.semana, REPLACE((SELECT IF(se.total_liquido > turno_1, 100, CAST((se.total_liquido / turno_1)*100 AS DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 1 AND semana = se.semana), '.', ',') AS plan_liquido_1, 
                                                        REPLACE((SELECT IF(IF(se.total_liquido - turno_1 < 0, 0, se.total_liquido - turno_1) > turno_2, 100, CAST((IF(se.total_liquido - turno_1 < 0, 0, se.total_liquido - turno_1)) / turno_2 * 100 AS DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 1 AND semana = se.semana), '.', ',') AS plan_liquido_2, 
                                                        REPLACE((SELECT CAST(IF(se.total_liquido - turno_1 - turno_2 < 0, 0, se.total_liquido - turno_1 - turno_2) / turno_3 * 100 AS  DECIMAL(4,2)) FROM capacidad_envasado WHERE id_linea = 1 AND semana = se.semana), '.', ',') AS plan_liquido_3, se.total_liquido,
                                                        REPLACE((SELECT IF(se.total_semi_solido > turno_1, 100, CAST((se.total_semi_solido / turno_1)*100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 2 AND semana = se.semana), '.', ',') AS plan_semi_solido_1, 
                                                        REPLACE((SELECT IF(IF(se.total_semi_solido - turno_1 < 0, 0, se.total_semi_solido - turno_1) > turno_2, 100, CAST((IF(se.total_semi_solido - turno_1 < 0, 0, se.total_semi_solido - turno_1)) / turno_2 * 100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 2 AND semana = se.semana), '.', ',') AS plan_semi_solido_2, 
                                                        REPLACE((SELECT CAST(IF(se.total_semi_solido - turno_1 - turno_2 < 0, 0, se.total_semi_solido - turno_1 - turno_2) / turno_3 * 100 AS  DECIMAL(4,2)) FROM capacidad_envasado WHERE id_linea = 2 AND semana = se.semana), '.', ',') AS plan_semi_solido_3, se.total_semi_solido,
                                                        REPLACE((SELECT IF(se.total_solido > turno_1, 100, CAST((se.total_solido / turno_1)*100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 3 AND semana = se.semana), '.', ',') AS plan_solido_1, 
                                                        REPLACE((SELECT IF(IF(se.total_solido - turno_1 < 0, 0, se.total_solido - turno_1) > turno_2, 100, CAST((IF(se.total_solido - turno_1 < 0 , 0, se.total_solido - turno_1)) / turno_2 * 100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 3 AND semana = se.semana), '.', ',') AS plan_solido_2, 
                                                        REPLACE((SELECT CAST(IF(se.total_solido - turno_1 - turno_2 < 0, 0, se.total_solido - turno_1 - turno_2) / turno_3 * 100 AS  DECIMAL(4,2)) FROM capacidad_envasado WHERE id_linea = 3 AND semana = se.semana), '.', ',') AS plan_solido_3, se.total_solido FROM capacidad_envasado_sum se WHERE se.semana >= WEEKOFYEAR(NOW())
                                                        UNION ALL
                                                        SELECT se.semana, REPLACE((SELECT IF(se.total_liquido > turno_1, 100, CAST((se.total_liquido / turno_1)*100 AS DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 1 AND semana = se.semana), '.', ',') AS plan_liquido_1, 
                                                        REPLACE((SELECT IF(IF(se.total_liquido - turno_1 < 0, 0, se.total_liquido - turno_1) > turno_2, 100, CAST((IF(se.total_liquido - turno_1 < 0, 0, se.total_liquido - turno_1)) / turno_2 * 100 AS DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 1 AND semana = se.semana), '.', ',') AS plan_liquido_2, 
                                                        REPLACE((SELECT CAST(IF(se.total_liquido - turno_1 - turno_2 < 0, 0, se.total_liquido - turno_1 - turno_2) / turno_3 * 100 AS  DECIMAL(4,2)) FROM capacidad_envasado WHERE id_linea = 1 AND semana = se.semana), '.', ',') AS plan_liquido_3, se.total_liquido,
                                                        REPLACE((SELECT IF(se.total_semi_solido > turno_1, 100, CAST((se.total_semi_solido / turno_1)*100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 2 AND semana = se.semana), '.', ',') AS plan_semi_solido_1, 
                                                        REPLACE((SELECT IF(IF(se.total_semi_solido - turno_1 < 0, 0, se.total_semi_solido - turno_1) > turno_2, 100, CAST((IF(se.total_semi_solido - turno_1 < 0, 0, se.total_semi_solido - turno_1)) / turno_2 * 100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 2 AND semana = se.semana), '.', ',') AS plan_semi_solido_2, 
                                                        ...
                                                        REPLACE((SELECT IF(se.total_solido > turno_1, 100, CAST((se.total_solido / turno_1)*100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 3 AND semana = se.semana), '.', ',') AS plan_solido_1, 
                                                        REPLACE((SELECT IF(IF(se.total_solido - turno_1 < 0, 0, se.total_solido - turno_1) > turno_2, 100, CAST((IF(se.total_solido - turno_1 < 0 , 0, se.total_solido - turno_1)) / turno_2 * 100 AS  DECIMAL(4,2))) FROM capacidad_envasado WHERE id_linea = 3 AND semana = se.semana), '.', ',') AS plan_solido_2, 
                                                        REPLACE((SELECT CAST(IF(se.total_solido - turno_1 - turno_2 < 0, 0, se.total_solido - turno_1 - turno_2) / turno_3 * 100 AS  DECIMAL(4,2)) FROM capacidad_envasado WHERE id_linea = 3 AND semana = se.semana), '.', ',') AS plan_solido_3, se.total_solido FROM capacidad_envasado_sum se
                                                    ) semanas LIMIT 12");
        $stmt->execute();
        $this->logger->info(__FUNCTION__, array('query' => $stmt->queryString, 'errors' => $stmt->errorInfo()));
