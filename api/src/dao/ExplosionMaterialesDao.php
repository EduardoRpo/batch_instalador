<?php
  ...
  class ExplosionMaterialesDao
  {
    ...
    {
      $connection = Connection::getInstance()->getConnection();
      $stmt = $connection->prepare("SELECT ep.id_materiaprima, m.nombre, COALESCE(SUM(eb.cantidad),0) AS cantidad_batch, COALESCE(SUM(ep.cantidad),0) AS cantidad_pedidos, COALESCE(SUM(eb.uso),0) AS cantidad_batch_uso, COALESCE(SUM(eb.cantidad),0) - COALESCE(SUM(eb.uso),0) AS gap_batch, COALESCE(SUM(ep.cantidad),0) - COALESCE(SUM(eb.uso),0) AS gap_pedidos 
                                    ...
                                    GROUP BY id_materiaprima ORDER BY `ep`.`id_materiaprima` ASC");
      $stmt->execute();
      $this->logger->info(__FUNCTION__, array('query' => $stmt->queryString, 'errors' => $stmt->errorInfo()));
      $batchExplosionMateriales = $stmt->fetchAll($connection::FETCH_ASSOC);
      $this->logger->notice("batch_explosion_materiales Obtenidos", array('batch_explosion_materiales' => $batchExplosionMateriales));
