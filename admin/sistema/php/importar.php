...
switch ($operacion) {
	case '1': // insertar en la BD condiciones
	...
	case '2': // insertar en la BD desinfectante
		foreach ($dataList as $data) {
	...
	case '3': // insertar en la BD equipos
		foreach ($dataList as $data) {
			$conn->query("INSERT INTO maquinaria (maquina, linea ) 
				  VALUES ('{$data[0]}', '{$data[1]}')");
		}
		break;
	case '4': // insertar en la BD procesos
		foreach ($dataList as $data) {
			$conn->query("INSERT INTO modulo (modulo) 
					  VALUES ('{$data[0]}')");
		}
		break;
	case '5': // insertar en la BD procesos
		foreach ($dataList as $data) {
