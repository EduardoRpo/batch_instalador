...
switch ($op) {
    case 1: //listar Condiciones del medio
        $query_cargos = mysqli_query($conn, "SELECT * FROM cargo");

        $result = mysqli_num_rows($query_cargos);

        mysqli_close($conn);

        if ($result > 0) {
            while ($data = mysqli_fetch_assoc($query_cargos)) {
                $arreglo["data"][] = $data;
            }
        ...
        mysqli_free_result($query_cargos);
