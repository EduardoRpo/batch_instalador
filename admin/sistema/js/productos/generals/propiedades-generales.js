...
function parametros(tabla, id) {
  ...
  if (tablaBD === "notificacion_sanitaria") cargarTablaNotificacion(id, tabla);
  else if (tablaBD === "linea") cargarTablaLinea(id, tabla);
  else cargarTablas(id, tabla);
}
...
function cargarTablaNotificacion(id, tabla) {
  $(id).DataTable({
    ...
    columns: [
      ...
      {
        defaultContent:
          "<a href='#' <i class='large material-icons link-editar' data-toggle='tooltip' title='Actualizar' style='color:rgb(255, 165, 0)'>edit</i></a>",
        className: "centrado",
      },
      {
        defaultContent:
...
$(document).on("click", ".link-editar", function (e) {
  ...
  if (
    (id_tbl === 4 && tablaBD !== "marca") ||
    (id_tbl === 5 && tablaBD !== "propietario") ||
    id_tbl === 8
  ) {
...
function guardarDatosGenerales(nombre, id) {
  ...
  if (nombre == "notificacion_sanitaria") vencimiento = $("#input21").val();

  if (nombre == "linea") densidad = $("#input31").val();

  if (
    !datos ||
    (nombre == "notificacion_sanitaria" && vencimiento == "") ||
    (nombre == "linea" && densidad == "")
  ) {
