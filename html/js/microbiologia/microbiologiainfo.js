      ...
      if (id != 'microbiologia_verificado') {
        ...
        if (continuar != 0) {
          ...
          for (let i = 0; i < cantMulti.length; i++) {
            ...
            if ($(`#inputMesofilos${i + 1}`).prop('disabled') == false) {
              ...
              if (mesofilos) {
                ...
                dataMicrobiologia.mesofilos = $(
                  `#inputMesofilos${i + 1}`
                ).val();
                dataMicrobiologia.pseudomona = $(`#pseudomona${i + 1}`).val();
                dataMicrobiologia.escherichia = $(`#escherichia${i + 1}`).val();
                dataMicrobiologia.staphylococcus = $(
                  `#staphylococcus${i + 1}`
                ).val();
                dataMicrobiologia.fechaSiembra = $(
                  `#fechaSiembra${i + 1}`
                ).val();
                dataMicrobiologia.fechaResultados = $(
                  `#fechaResultados${i + 1}`
                ).val();
          ...
          for (let i = 2; i < dataMicro.length; i++) {
            if (
              !dataMicro[i].mesofilos ||
              !dataMicro[i].pseudomona ||
              !dataMicro[i].escherichia ||
              !dataMicro[i].staphylococcus ||
              !dataMicro[i].fechaSiembra ||
              !dataMicro[i].fechaResultados
            ) {
              alertify.set('notifier', 'position', 'top-right');
              alertify.error(
                'Seleccione e Ingrese los datos del análisis Microbiológico'
    ...
    $.ajax({
      ...
      success: function (r) {
        if (r == 'false') {
          alertify.set('notifier', 'position', 'top-right');
          alertify.error(
            `No es posible cerrar este proceso para el Batch ${idBatch}. El módulo de fisicoqumico aún no se encuentran completamente firmado`
          );
          return false;
        }
