/* consulta Equipos para selects */

$.ajax({
    url: `/api/equipos`,
    type: "GET",
}).done((data, status, xhr) => {
    $(".sel_equipos").append(`<option value="0" selected disabled>Seleccionar</option>`);
...
$(document).ready(function() {
    ...
    validarEquipos = () => {
        ...
        if (equipo1 * equipo2 == 0) {
            ...
            alertify.error("Antes de continuar, seleccione los Equipos a usar para la linea de producción");
            return 0;
        }
    }
    ...
    ajaxAlmacenarEquipos = async() => {
        data = { equipos }
        return result = await sendDataPOST("../../html/php/equipos.php", data)
    }

    almacenarEquipos = async() => {
        response = await ajaxAlmacenarEquipos()
