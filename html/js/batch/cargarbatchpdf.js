  ...
  $.get(`/api/batch/${idBatch}`, function (data, textStatus, jqXHR) {
    if ((textStatus = 'success')) {
      if (data) {
        ...
        $('#accions').empty();
        $('#accions').append(
          `<a href="/pdf/${idBatch}/${data.referencia}" target="_blank"><i id="${idBatch}" class="fas fa-external-link-alt downloadExcelBatch"></i></a>`
        );

        if (data.estado == 0) $('#titleBatchEliminado').show();
        else $('#titleBatchEliminado').hide();
      } else {
