...
try {
    ...
    // Mapear columnas para aprobación
    $columns = ['id_batch', 'fecha_programacion', 'numero_orden', 'referencia', 'nombre_referencia', 'numero_lote', 'cantidad_firmas', 'total_firmas'];
    $order_by = $columns[$order_column] ?? 'id_batch';
    
    // Construir consulta base para aprobación
    $sql = "SELECT batch.id_batch, batch.fecha_programacion, batch.numero_orden, batch.id_producto as referencia, 
                   p.nombre_referencia, batch.numero_lote, batch.estado, bcf.cantidad_firmas, bcf.modulo, bcf.total_firmas   
            FROM batch 
