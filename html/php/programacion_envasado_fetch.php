...
try {
    ...
    // Mapear columnas para programación envasado
    $columns = ['id_batch', 'estado', 'referencia', 'pedido', 'fecha_envasado', 'propietario', 'nombre_referencia', 'numero_lote', 'unidad_lote', 'tamano_lote'];
    $order_by = $columns[$order_column] ?? 'id_batch';
    
    // Construir consulta base para programación envasado
    $sql = "SELECT batch.id_batch, batch.estado, batch.id_producto as referencia, 
                   batch.numero_orden as pedido, batch.programacion_envasado as fecha_envasado,
                   p.propietario, p.nombre_referencia, batch.numero_lote, 
    ...
    if ($fecha) {
        $sql .= " AND DATE(batch.programacion_envasado) = :fecha";
        $count_sql .= " AND DATE(batch.programacion_envasado) = :fecha";
    }
