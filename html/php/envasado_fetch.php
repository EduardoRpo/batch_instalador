...
try {
    ...
    $columns = ['id_batch', 'programacion_envasado', 'referencia', 'nombre_referencia', 'unidad_lote', 'numero_lote', 'cantidad_firmas', 'total_firmas', 'cant_observations'];
    $order_by = $columns[$order_column] ?? 'id_batch';
    
    // Construir consulta base para envasado
    $sql = "SELECT batch.id_batch, batch.programacion_envasado, batch.id_producto as referencia, 
                   p.nombre_referencia, batch.unidad_lote, batch.numero_lote, batch.estado, 
                   bcf.cantidad_firmas, bcf.modulo, bcf.total_firmas,
                   COALESCE(obs.cant_observations, 0) as cant_observations
            FROM batch 
            INNER JOIN producto p ON p.referencia = batch.id_producto
            INNER JOIN batch_control_firmas bcf ON bcf.batch = batch.id_batch  
            LEFT JOIN (
                SELECT id_batch, COUNT(*) as cant_observations 
                FROM observaciones 
                GROUP BY id_batch
            ) obs ON obs.id_batch = batch.id_batch
