...
switch ($op) {
  ...
  case 3: //cargar selector de referencias
    ...
    $query_referencia = mysqli_query($conn, "SELECT @curRow := @curRow + 1 AS id, CONCAT (referencia, ' ' ,nombre_referencia) AS referencia 
                                            FROM producto 
  ...
  case 5: // Guardar
    ...
    if ($result) {
      ...
      $query_id = mysqli_query($conn, "SELECT MAX(id_batch) AS id FROM batch");
