...
if (!empty($_POST)) {
    ...
    switch ($op) {
        ...
        case 3:
            $sql = "SELECT bf2.observaciones as linea, bf2.modulo, bf2.batch, u.urlfirma, bf2.verifico FROM batch_firmas2seccion bf2 INNER JOIN usuario U ON U.id=bf2.id  WHERE modulo = :modulo AND batch = :batch AND ref_multi = :ref_multi";
            $query = $conn->prepare($sql);
            $result = $query->execute([
                'modulo' => $modulo,
                'batch' => $batch,
